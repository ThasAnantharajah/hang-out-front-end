  ...
  const fetchMessages = async () => {
    try {
      const response = await fetch(
        `${BACK_END_URL}/messages/discution/${senderId}/${receiverId}`
      );
  ...
  useFocusEffect(
    React.useCallback(() => {
      ...
      return () => {
        ...
        if (socket) {
          socket.emit("stoppedViewingMessages", {
            userId: senderId,
            receiverId,
          });
        }
  ...
  const markMessageAsRead = async (messageId) => {
    try {
      ...
      if (response.ok) {
        ...
        console.error(
          "MessageScreen Error marking message as read:",
          response.statusText
        );
      }
    } catch (error) {
  ...
  const sendMessage = () => {
    if (socket && newMessage) {
      ...
      setMessages((prevMessages) => [
        ...prevMessages,
        { ...messageData, _id: Date.now(), timestamp: Date.now() },
      ]); // Ajouter le message localement pour l'affichage
          ...
          <FlatList
            ...
            renderItem={({ item, index }) =>
              item.sender === senderId ? (
                <View
                  style={styles.messageSendContainer}
                  key={`${item._id}-${index}`}
                >
                ...
                <View
                  style={styles.messageReceiveContainer}
                  key={`${item._id}-${index}`}
                >
          ...
          <View style={styles.sendItems}>
            ...
            <View style={{ paddingTop: 5, marginRight: 10 }}>
              <TouchableOpacity
                ...
                onPress={sendMessage}
              >
                <FontAwesome style={{ color: "white" }} name="send" size={20} />
              </TouchableOpacity>
            </View>
          </View>
        </KeyboardAvoidingView>
